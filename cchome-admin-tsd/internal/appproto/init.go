...
type VerifyCodeReq struct {
	...
	Op    int    `json:"op"`    // 1 注册请求验证码, 2 忘记密码请求验证码, 3 删除账户
	// 二选一 必须得有
...
type UserLoginReq struct {
	LoginType uint8 `json:"login_type"` // 登录方式  0 邮箱登录 1 手机号登录 2 appleid登录 3 google登录, 4 微信小程序登陆
	...
	WXLoginInfo     *WXLoginInfo     `json:"wx_login_info"`     // wx登陆
...
type UserLoginReply struct {
	...
	Email             string `json:"email"`
	ManualCtrlConnect bool   `json:"manual_ctrl_connect"`
	FID               string `json:"fid"`
	Apikey            string `json:"apikey"`
}

type GetApikeyInfoReq struct {
}
type GetApikeyInfoReply struct {
	FID    string `json:"fid"`
	Apikey string `json:"apikey"`
}
...
type EvseAuthCodeReq struct {
	SN string `json:"sn"` //	桩号
}
type EvseAuthCodeReply struct {
	AuthCode  string `json:"auth_code"`
	TotalTime uint32 `json:"total_time"` // 总时间 秒
	TTL       uint32 `json:"ttl"`        // 剩余时间 秒
}

type StatisticsReq struct {
	Time string `json:"time"` // "2023-09"
}

type StatisticsInfo struct {
	Time     string  `json:"time"`        // "2023-09"
	KWh      float64 `json:"kwh"`         // 电量
	TotalNum uint32  `json:"total_order"` // 总订单数量
}
type StatisticsReply struct {
	StatisticsInfos []StatisticsInfo `json:"statistics_infos"`
}
type SetProfileReq struct {
	AlertNotify        bool   `json:"auth_code"`
	StopChargingNotify bool   `json:"stop_charging_notify"`
	Price              uint32 `json:"price"` // 分， 货币单位随用户自定义
}
...
type GetProfileReply struct {
	AlertNotify        bool   `json:"enable_alert_notify"`
	StopChargingNotify bool   `json:"enable_stop_charging_notify"`
	Price              uint32 `json:"price"` // 分， 货币单位随用户自定义
}

type UserBindEvseReq struct {
	SN         string `json:"sn"`          //	桩号
	IOSMac     string `json:"ios_mac"`     // ios mac地址  两个mac地址格式不一样
	AndroidMac string `json:"android_mac"` // android mac地址
...
type EvseStaticData struct {
	...
	AndroidMac      string `json:"android_mac"`       // android mac地址
	Alias           string `json:"alias"`             //
	NetWork         int    `json:"network"`           // 联网方式 0：WIFI 1：4G
}

type BindEvseInfo struct {
	EvseStaticData
	State          int  `json:"state"`           // 桩状态 0  1 离线 2 在线 3 故障
	Status         int  `json:"status"`          //  0 未知, （说明设备未上线过） 1 离线 2 在线 3 故障
	IsMaster       bool `json:"is_master"`       // 是否已经绑定
...
type EvseInfoReply struct {
	...
	SupportBindMode int    `json:"support_bind_mode"` // 0 直接绑定, 1 授权绑定
	NetWork         int    `json:"network"`           // 联网方式 0：WIFI 1：4G
}
