define(['fast', 'template', 'moment'], function (Fast, Template, Moment) {
    ...
    var Backend = {
        api: {
            ...
            reload: function(){
                $("iframe",window.top.document).each(function (i,v) {
                    var t = $(this);
                   if(t.parent("div").hasClass("active")){
                       var url = window.top.document.URL;
                       url = url.replace("addtabs=1", "");
                       url = url.replace("ref=addtabs", "addtabs=1");
                       var mao = url.indexOf("#");
                       if (mao>=0) {
                           url = url.substr(0, mao);
                       }
                       t.attr("src",url);
                       return false;
                   }
        ...
        init: function () {
            if(self.window != parent.window){
                $("ul.nav li a").click(function(){
                    ...
                    parent.window.history.pushState({},0,href);
                });
            }

            //     ,window.frames.document).click(function(){
            //     parent.window.history.pushState({},0,'http://'+window.location.host+$(this).attr('href'));
            // });
            //公共代码
            ...
            $(document).on('click', '.btn-dialog,.dialogit', function (e) {
                ...
                if($(that).hasClass("btn-dialog-shade")) {
                    options.shade = 0.5;
                }
